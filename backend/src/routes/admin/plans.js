...
export default async function planRoutes(fastify) {
  ...
  fastify.post('/', async (req, reply) => {
    const { name, cpu, ram, disk, bandwidth, price, type } = req.body || {};
    if (!name || !cpu || !ram || !disk) return reply.status(400).send({ success: false, error: 'name, cpu, ram, disk required' });
    const r = await query('INSERT INTO plans (name, cpu, ram, disk, bandwidth, price, type) VALUES (?, ?, ?, ?, ?, ?, ?)',
  ...
  fastify.put('/:id', async (req, reply) => {
    const { name, cpu, ram, disk, bandwidth, price, type, is_active } = req.body || {};
    await query('UPDATE plans SET name=COALESCE(?,name), cpu=COALESCE(?,cpu), ram=COALESCE(?,ram), disk=COALESCE(?,disk), bandwidth=COALESCE(?,bandwidth), price=COALESCE(?,price), type=COALESCE(?,type), is_active=COALESCE(?,is_active) WHERE id=?',
      [name??null, cpu??null, ram??null, disk??null, bandwidth??null, price??null, type??null, is_active??null, req.params.id]);
