...
export default async function nodeRoutes(fastify) {
  ...
  fastify.get('/', async (req, reply) => {
    const nodes = await query('SELECT id, name, hostname, port, proxmox_node, storage, backup_storage, type, location, total_cpu, total_ram, total_disk, is_active, created_at FROM nodes ORDER BY created_at DESC');
    return reply.send({ success: true, data: nodes });
  });

  fastify.post('/', async (req, reply) => {
    const { name, hostname, port, api_token_id, api_token_secret, proxmox_node, storage, backup_storage, type, location, total_cpu, total_ram, total_disk } = req.body || {};
    ...
    const result = await query(
      'INSERT INTO nodes (name, hostname, port, api_token_id, api_token_secret, proxmox_node, storage, backup_storage, type, location, total_cpu, total_ram, total_disk) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
      [name, hostname, port || 8006, api_token_id, api_token_secret, proxmox_node || 'pve', storage || 'local-lvm', backup_storage || 'local', type || 'both', location || '', total_cpu || 0, total_ram || 0, total_disk || 0]
    );
  ...
  fastify.put('/:id', async (req, reply) => {
    const { name, hostname, port, api_token_id, api_token_secret, proxmox_node, storage, backup_storage, type, location, total_cpu, total_ram, total_disk, is_active } = req.body || {};
    await query(
      ...
      [name??null, hostname??null, port??null, api_token_id??null, api_token_secret??null, proxmox_node??null, storage??null, backup_storage??null, type??null, location??null, total_cpu??null, total_ram??null, total_disk??null, is_active??null, req.params.id]
    );
